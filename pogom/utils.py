...
def get_args():
    ...
    parser = configargparse.ArgParser(
        ...
        auto_env_var_prefix='RM_',
        add_env_var_help=False,
        formatter_class=configargparse.ArgumentDefaultsRawHelpFormatter)
    group = parser.add_argument_group('general')
    group.add_argument(
        '-cf', '--config', is_config_file=True, help='Set configuration file')
    group.add_argument(
        '-a',
        '--auth-service',
        type=str.lower,
        ...
        help=('Auth Services, either one for all accounts ' +
              'or one per account: ptc or google. Defaults all to ptc.'))
    group.add_argument(
        '-u',
        '--username',
        action='append',
    ...
    group.add_argument(
        ...
        action='append',
        default=[],
        help=('Passwords, either single one for all accounts or' +
              'one per account.'))
    group.add_argument(
        '-ac',
        '--accountcsv',
    ...
    group.add_argument(
        '-st', '--step-limit', help='Steps.', type=int, required=True)
    # geofence
    group = parser.add_argument_group('geofence')
    group.add_argument(
        '-gf',
        '--geofence-file',
        help='Geofence file to define outer borders of the scan area.',
        default='')
    group.add_argument(
        '-gef',
        '--geofence-excluded-file',
        help=('File to define excluded areas inside scan area. Regarded this' +
              ' as inverted geofence. Can be combined with geofence-file.'),
        default='')
    # db
    group = parser.add_argument_group('database')
    group.add_argument(
        '--db-type',
        help='Type of database to be used.',
        choices=['sqlite', 'mysql'],
        default='sqlite')
    group.add_argument(
        '-D', '--db', help='Database filename for SQLite.', default='pogom.db')
    ...
    group.add_argument(
        '--db-max_connections',
        help='Max connections for the database.',
        type=int,
        default=5)
    group.add_argument(
        '--db-threads',
        ...
        default=1)
    # Scan modifiers
    group = parser.add_argument_group('map detail')
    group.add_argument(
        '-np',
        '--no-pokemon',
        help=('Disables Pokemon from the map (including parsing them' +
              ' into local db.)'),
        action='store_true',
        default=False)
    group.add_argument(
    ...
    group.add_argument(
        ...
        '--ignorelist-file',
        default='',
        help=('File containing a list of Pokemon IDs to ignore, one line' +
              ' per ID. Spawnpoints will be saved, but ignored Pokemon ' +
              ' won\'t be encountered, sent to webhooks or saved to the DB.'))
    # schedulers
    group = parser.add_argument_group('scheduler')
    scheduler = group.add_mutually_exclusive_group()
    scheduler.add_argument(
        '-ss',
        '--spawnpoint-scanning',
        help=('Use spawnpoint scanning. Scans in a circle based on' +
              ' step_limit when on DB.'),
        nargs='?',
        const='nofile',
        default=False)
    scheduler.add_argument(
        '-speed',
        '--speed-scan',
        help=('Use speed scanning to identify spawn' +
              ' points and then scan closest spawns.'),
        action='store_true',
        default=False)
    # schedulers params
    group.add_argument(
        '-bh',
        '--beehive',
        help=('Use beehive configuration for multiple accounts, one' +
              ' account per hex.  Make sure to keep -st under 5, and' +
              ' -w under the total amount of accounts available.'),
        action='store_true',
        default=False)
    group.add_argument(
        '-wph',
        ...
        help=('Only referenced when using --beehive. Sets ' +
              'number of workers per hive'),
        type=int,
        default=1)
    group.add_argument(
        '-kph',
        '--kph',
        help='Set a maximum speed in km/hour for scanner movement.',
        type=int,
        default=35)
    group.add_argument(
        '--skip-empty',
        help=('Enables skipping of empty cells in normal scans - requires' +
              ' previously populated database (not to be used with -ss)'),
        action='store_true',
        default=False)
    group.add_argument(
        '-bsr',
        '--bad-scan-retry',
        help='Number of bad scans before giving up on a step. 0 to disable.',
        type=int,
        default=2)
    group.add_argument(
        '-msl',
        '--min-seconds-left',
        help=('Time that must be left on a spawn before considering it too' +
              ' late and skipping it. For example 600 would skip anything'
    ...
    group.add_argument(
        ...
        help=('Number of seconds after spawn time to wait ' +
              'before scanning to be sure the Pokemon is there.'),
        type=float,
        default=10)
    group.add_argument(
        '-nj',
        '--no-jitter',
        help=("Don't apply random -9m to +9m jitter to location."),
        action='store_true',
        default=False)
    # account management
    group = parser.add_argument_group('account management')
    group.add_argument(
        '-mf',
        '--max-failures',
        help=('Maximum number of failures to parse locations before an' +
    ...
    group.add_argument(
        '-me',
        '--max-empty',
        help=('Maximum number of empty scans before an account will go into' +
              ' a sleep for -ari/--account-rest-interval seconds.' +
    ...
    group.add_argument(
        ...
        type=int,
        help='Seconds for accounts to rest when they put to sleep.',
        default=7200)
    group.add_argument(
        '-ld',
        '--login-delay',
    ...
    group.add_argument(
        ...
        default=80)
    # encounter
    group = parser.add_argument_group('encounters')
    group.add_argument(
        '-enc',
        '--encounter',
    ...
    group.add_argument(
        ...
        '--enc-whitelist-file',
        default='',
        help=('File containing a list of Pokemon IDs to encounter for' +
              ' IV/CP scanning.'))
    group.add_argument(
        '-hlvl',
        '--high-lvl-accounts',
        help=('Load high level accounts from CSV file  containing ' +
              '"auth_service,username,passwd" lines.'))
    group.add_argument(
        '-hkph',
        '--hlvl-kph',
    ...
    group.add_argument(
        '-nostore',
        '--no-api-store',
        help=('Don\'t store the API objects used by the high level accounts' +
              ' in memory. This will increase the number of logins per' +
              ' account, but decreases memory usage.'),
        action='store_true',
        default=False)
    # server
    group = parser.add_argument_group('web server')
    server = group.add_mutually_exclusive_group()
    server.add_argument(
        '-ns',
        '--no-server',
    ...
    server.add_argument(
        '-os',
        '--only-server',
        help=('Server-Only Mode. Starts only the Webserver without' +
              ' the searcher.'),
        action='store_true',
        default=False)
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=True)
    group.add_argument(
        '--ssl-certificate', help='Path to SSL certificate file.')
    group.add_argument(
        '--ssl-privatekey', help='Path to SSL private key file.')
    group.add_argument(
        '-C',
        '--cors',
    ...
    group.add_argument(
        ...
        help=('Locale for Pokemon names (default: {}, check ' +
              '{} for more).').format(config['LOCALE'], config['LOCALES_DIR']),
        default='en')
    # status
    group = parser.add_argument_group('status/console')
    group.add_argument(
        '-ps',
        '--print-status',
    ...
    group.add_argument(
        ...
        default=False)
    # web hooks
    group = parser.add_argument_group('webhooks')
    group.add_argument(
        '-wh',
        '--webhook',
    ...
    group.add_argument(
        '-whc',
        '--wh-concurrency',
        help='Async requests pool size.',
        type=int,
        default=25)
    group.add_argument(
        '-whr',
        '--wh-retries',
        help='Number of times to retry sending webhook data on failure.',
        type=int,
        default=3)
    group.add_argument(
        '-wht',
    ...
    group.add_argument(
        '-whbf',
        '--wh-backoff-factor',
        help=('Factor (in seconds) by which the delay until next retry' +
              ' will increase.'),
        type=float,
        default=0.25)
    group.add_argument(
        '-whlfu',
        '--wh-lfu-size',
        help='Webhook LFU cache max size.',
        type=int,
        default=2500)
    webhook_list = group.add_mutually_exclusive_group()
    webhook_list.add_argument(
        '-wwht',
        '--webhook-whitelist',
        action='append',
        default=[],
        help='List of Pokemon to send to webhooks. Specified as Pokemon ID.')
    webhook_list.add_argument(
        '-wblk',
        '--webhook-blacklist',
        action='append',
        default=[],
        help=('List of Pokemon NOT to send to webhooks. Specified as' +
              ' Pokemon ID.'))
    webhook_list.add_argument(
        '-wwhtf',
        '--webhook-whitelist-file',
        default='',
        help='File containing a list of Pokemon IDs to be sent to webhooks.')
    webhook_list.add_argument(
        '-wblkf',
        '--webhook-blacklist-file',
        default='',
        help=('File containing a list of Pokemon IDs NOT to be sent' +
              ' to webhooks.'))
    # altitude
    group = parser.add_argument_group('altitude')
    group.add_argument(
        '-altv',
        '--altitude-variance',
    ...
    group.add_argument(
        ...
        help=('Query the Elevation API for each step,' +
              ' rather than only once, and store results in the database.'),
        action='store_true',
        default=False)
    # captcha
    group = parser.add_argument_group('captcha')
    group.add_argument(
        '-cs',
        '--captcha-solving',
        ...
        action='store_true',
        default=False)
    group.add_argument('-ck', '--captcha-key', help='2Captcha API key.')
    group.add_argument(
        '-cds',
        '--captcha-dsk',
    ...
    group.add_argument(
        ...
        help=('Maximum time captchas will wait for manual captcha solving.' +
              ' On timeout, if enabled, 2Captcha will be used to' +
              ' solve captcha.'),
        type=int,
        default=0)
    # proxy
    ...
    group.add_argument(
        '-pxre',
        '--proxy-test-retries',
        help=('Number of times to retry sending proxy test requests' +
              ' on failure.'),
        type=int,
        default=0)
    group.add_argument(
    ...
    group.add_argument(
        '-pxd',
        '--proxy-display',
        help='Display info on which proxy being used. To be used with -ps.',
        type=str,
        choices=['index', 'full'],
        default='index')
    group.add_argument(
    ...
    group.add_argument(
        ...
        help=('Period of proxy file reloading, in seconds. ' +
              'Works only with -pxf/--proxy-file. (0 to disable).'),
        type=int,
        default=0)
    group.add_argument(
        '-pxo',
        ...
        type=str,
        default='round',
        choices=['none', 'round', 'random'])
    # db cleaning
    group = parser.add_argument_group('database maintenance')
    group.add_argument(
        '-pd',
        '--purge-data',
    ...
    group.add_argument(
        ...
        default=False)
    # anti-scrapper
    group = parser.add_argument_group('anti scrappers (web server)')
    group.add_argument(
        '--disable-blacklist',
    ...
    group.add_argument(
        ...
        help=('Change duration for lures set on pokestops. ' +
              'This is useful for events that extend lure duration.'),
        type=int,
        default=30)
    group.add_argument(
        '-c',
    ...
    group.add_argument(
        ...
        help=('Mock mode - point to a fpgo endpoint instead ' +
              'of using the real PogoApi, ec: http://127.0.0.1:9090'),
        default='')
    group.add_argument(
        '-apir',
        '--api-retries',
    ...
    group.add_argument(
        ...
        type=int,
        help='Interval to check API version in seconds (use 0 to disable).',
        default=random.randint(60, 300))
    # debug
    group = parser.add_argument_group('debug')
    verbose = group.add_mutually_exclusive_group()
    verbose.add_argument(
        '-v',
        help=('Show debug messages from RocketMap and pgoapi.' +
